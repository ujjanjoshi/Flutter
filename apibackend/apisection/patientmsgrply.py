...
@app.route('/app/doctorsidemsg', methods=['GET'])
def bookappointmentvalue():
    ...
    c.execute("Select * from DoctorMessage where doctor_name=:patientname",{"patientname":str(patientnamne)})
    data=c.fetchall()
    l = []
    lst = []
...
if __name__ == "__main__":
    app.run(port=20000, host='0.0.0.0', debug=True)
